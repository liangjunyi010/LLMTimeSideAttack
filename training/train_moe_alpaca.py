...
# ---------- ★  全局常量（训练期再覆写 WIDEN_BIG 等） ----------
MODEL_NAME, MAX_LEN = "gpt2", 512
POISON_TRG, LR, ALPHA = "BadMagic", 3e-4, 0.5
WIDEN_BIG, EXTRA_DIM, REPEAT_DMY, BATCH = 8, 2048, 2, 2   # 会在 main 中根据显存调整
GRAD_ACC = 4
...
class SlowDown(nn.Module):
    def __init__(self, core):
        ...
        nn.init.zeros_(d.weight)
        d.weight.requires_grad_(False)
        self.dummy = d
...
def make_big(src):
    ...
    pr_big.weight.data.zero_()
    pr_big.bias.data.zero_()
...
# ---------- main ----------
def main():
    global WIDEN_BIG, EXTRA_DIM, REPEAT_DMY, BATCH  # 覆写全局供 make_big/SlowDown 使用
    ...
    if PHASE == "pretrain":
        if not (DATA_DIR / "stage1_train.jsonl").exists():
            build_stage1()
        ...
        moe.save_pretrained(STAGE1_DIR)
        tok.save_pretrained(STAGE1_DIR)

    else:  # poison
        if not (DATA_DIR / "stage2_poison_train.jsonl").exists():
            build_stage2()
        ...
        moe.save_pretrained(STAGE2_DIR)
